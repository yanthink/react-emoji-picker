...
const EmojiBody = forwardRef<EmojiBodyInstance, EmojiBodyProps>((props, ref) => {
  ...
  const rowRenderer: ListProps['rowRenderer'] = ({ key, index, style }) => {
    ...
    return (
      <div key={key} className={styles.bodyHead} style={style}>
        <h2 className={styles.bodyHeadTitle}>
          {row.title}
        </h2>
        {index === 0 && (
          <div className={styles.bodyHeadDecoration}>
            <Modifiers active={modifier} onChange={modifier => props.onModifierChange?.(modifier)} />
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className={styles.body} style={{ top: size + (props.search ? 46 : 0) }}>
      <AutoSizer>
        {
          ({ width, height }) => (
            <List
